...
class TestTransactionService < Test::Unit::TestCase
  ...
  class << self
    def startup
      ...
      @@token_service = PostFinanceCheckout::TokenService.new
    end
  end
  ...
  def test_create
    ...
    assert_equal(PostFinanceCheckout::TransactionState::PENDING, transaction.state,
  ...
  def test_confirm
    ...
    assert_equal(PostFinanceCheckout::TransactionState::CONFIRMED, transaction_confirmed.state,
  ...
  def test_create_complete_immediately
    ...
    transaction_create.tokenization_mode = PostFinanceCheckout::TokenizationMode::FORCE_CREATION
    transaction_create.customers_presence = PostFinanceCheckout::CustomersPresence::NOT_PRESENT
    transaction_create.completion_behavior = PostFinanceCheckout::TransactionCompletionBehavior::COMPLETE_IMMEDIATELY

    transaction = @@transaction_service.create(SPACE_ID, transaction_create)
    transaction_processed = @@card_processing_service.process(SPACE_ID, transaction.id, 
      TEST_CARD_PAYMENT_METHOD_CONFIGURATION_ID, FAKE_CARD_DATA)

    assert_equal(PostFinanceCheckout::TransactionState::FULFILL, transaction_processed.state,
  ...
  def test_read
    ...
    assert_equal(PostFinanceCheckout::TransactionState::PENDING, transaction_read.state,
      "State must be PENDING")
  end
  ...
  def test_search
    ...
    entity_query = PostFinanceCheckout::EntityQuery.new({
      filter: PostFinanceCheckout::EntityQueryFilter.new({
        fieldName: "id",
        value: transaction.id,
        type: PostFinanceCheckout::EntityQueryFilterType::LEAF,
  ...
  def test_process_one_click_token_and_redirect_with_credentials
    ...
    transaction1_create.tokenization_mode = PostFinanceCheckout::TokenizationMode::FORCE_CREATION_WITH_ONE_CLICK_PAYMENT
    transaction1_create.customers_presence = PostFinanceCheckout::CustomersPresence::NOT_PRESENT
    transaction1_create.completion_behavior = PostFinanceCheckout::TransactionCompletionBehavior::COMPLETE_DEFERRED
    ...
    transaction2_create.tokenization_mode = PostFinanceCheckout::TokenizationMode::FORCE_CREATION_WITH_ONE_CLICK_PAYMENT
    transaction2_create.customers_presence = PostFinanceCheckout::CustomersPresence::NOT_PRESENT
    transaction2_create.completion_behavior = PostFinanceCheckout::TransactionCompletionBehavior::COMPLETE_DEFERRED
    ...
    assert_equal(PostFinanceCheckout::TransactionState::AUTHORIZED, transaction2_read.state, 
      "State must be AUTHORIZED")

    # cleanup - 2nd transaction
    transaction2_completed = @@transaction_completion_service.complete_online(SPACE_ID, transaction2.id)

    assert_equal(PostFinanceCheckout::TransactionCompletionState::SUCCESSFUL, transaction2_completed.state,
    ...
    assert_equal(PostFinanceCheckout::TransactionCompletionState::SUCCESSFUL, transaction1_completed.state,
      "State must be SUCCESSFUL")
