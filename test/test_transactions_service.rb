...
# postfinancecheckout transactions service tests
class TransactionsServiceTest < Test::Unit::TestCase

  @@INTEGRATION_MODE = 'payment_page';

  def setup
    @transactions_service = PostfinancecheckoutRubySdk::TransactionsService.new()
    @tokens_service = PostfinancecheckoutRubySdk::TokensService.new()
  end
  ...
  def test_create_and_find_pending_transaction
    ...
    assert_equal(
      PostfinancecheckoutRubySdk::TransactionState::PENDING,
      transaction.state,
  ...
  def test_confirm_should_make_transaction_confirmed
    ...
    transaction_pending = PostfinancecheckoutRubySdk::TransactionPending.new(version: 2)
    ...
    assert_equal(
      PostfinancecheckoutRubySdk::TransactionState::CONFIRMED,
      confirmed.state,
  ...
  def test_process_deferred_transaction_should_make_transaction_authorized
    transaction_create = TestUtils.get_transaction_create_payload
    transaction_create.tokenization_mode = PostfinancecheckoutRubySdk::TokenizationMode::FORCE_CREATION
    transaction_create.customers_presence = PostfinancecheckoutRubySdk::CustomersPresence::NOT_PRESENT
    transaction_create.completion_behavior = PostfinancecheckoutRubySdk::TransactionCompletionBehavior::COMPLETE_DEFERRED
    ...
    assert_equal(
      PostfinancecheckoutRubySdk::TransactionState::AUTHORIZED,
      processed.state,
  ...
  def test_process_via_charge_flow_should_make_transaction_processing
    ...
    assert_equal(
      PostfinancecheckoutRubySdk::TransactionState::PROCESSING,
  ...
  def test_cancel_charge_flow_should_make_transaction_failed
    ...
    assert_equal(
      PostfinancecheckoutRubySdk::TransactionState::FAILED,
  ...
  def test_complete_online_should_make_transaction_completion_state_successful
    ...
    transaction_create.tokenization_mode = PostfinancecheckoutRubySdk::TokenizationMode::FORCE_CREATION
    transaction_create.customers_presence = PostfinancecheckoutRubySdk::CustomersPresence::NOT_PRESENT
    transaction_create.completion_behavior = PostfinancecheckoutRubySdk::TransactionCompletionBehavior::COMPLETE_IMMEDIATELY
    ...
    assert_equal(
      PostfinancecheckoutRubySdk::TransactionCompletionState::SUCCESSFUL,
    ...
    assert_equal(
      PostfinancecheckoutRubySdk::TransactionState::FULFILL,
  ...
  def test_complete_online_partially_should_make_transaction_completion_state_successful
    ...
    tcd = PostfinancecheckoutRubySdk::TransactionCompletion.new
  ...
  def test_void_transaction_online_should_return_voided_transaction
    ...
    assert_equal(
      PostfinancecheckoutRubySdk::TransactionVoidState::SUCCESSFUL,
    ...
    assert_equal(
      PostfinancecheckoutRubySdk::TransactionState::VOIDED,
  ...
  def test_fetch_with_credentials_with_given_bad_credentials_should_fail
    assert_raises(PostfinancecheckoutRubySdk::ApiError, "Bad token should error response") do
  ...
  def test_update_should_change_transaction_data
    ...
    update = PostfinancecheckoutRubySdk::TransactionPending.new(version: 2, language: "en-GB")
  ...
  def test_process_one_click_token_and_redirect_with_credentials_should_return_payment_url
    ...
    token_create = PostfinancecheckoutRubySdk::TokenCreate.new(external_id: SecureRandom.uuid)
    token_create.state = PostfinancecheckoutRubySdk::CreationEntityState::ACTIVE
    ...
    token_update = PostfinancecheckoutRubySdk::TokenUpdate.new(enabled_for_one_click_payment: true, version: token.version)
    ...
    line_item = PostfinancecheckoutRubySdk::LineItemCreate.new(
      name: 'Red T-Shirt',
      unique_id: '5412',
      type: 'PRODUCT',
      quantity: 1,
    ...
    transaction1_pending = PostfinancecheckoutRubySdk::TransactionPending.new(
    ...
    transaction2_create.tokenization_mode = PostfinancecheckoutRubySdk::TokenizationMode::FORCE_CREATION_WITH_ONE_CLICK_PAYMENT
    transaction2_create.customers_presence = PostfinancecheckoutRubySdk::CustomersPresence::NOT_PRESENT
    transaction2_create.completion_behavior = PostfinancecheckoutRubySdk::TransactionCompletionBehavior::COMPLETE_DEFERRED
    ...
    assert_not_equal(read_transaction.state, PostfinancecheckoutRubySdk::TransactionState::FAILED, "Transaction state should not be FAILED")
    assert_not_equal(read_transaction.state, PostfinancecheckoutRubySdk::TransactionState::PENDING, "Transaction state should not be PENDING")

    completed_transaction1 = @transactions_service.post_payment_transactions_id_complete_online(
      transaction1.id, SPACE_ID
    )

    assert_equal(completed_transaction1.state, PostfinancecheckoutRubySdk::TransactionCompletionState::SUCCESSFUL, "State must be SUCCESSFUL")
  end
  ...
  def test_process_transaction_via_token_should_process_transaction
    ...
    transaction_pending = PostfinancecheckoutRubySdk::TransactionPending.new(
      token: token.id,
    ...
    assert_equal(read_transaction.state, PostfinancecheckoutRubySdk::TransactionState::FULFILL, "Transaction state must be FULFILLED")
  end
  ...
  def test_process_without_user_interaction_should_process_transaction_properly
    ...
    assert_equal(processed.state, PostfinancecheckoutRubySdk::TransactionState::PROCESSING, "Transaction state should be PROCESSING")
  end
  ...
  def test_process_transaction_with_three_d_secure_should_fulfill_transaction
    transaction = create_transaction(TestUtils.get_transaction_create_payload)
    tokenized_card_data_create = PostfinancecheckoutRubySdk::TokenizedCardDataCreate.new(primary_account_number: MOCK_CARD_DATA.card_data.primary_account_number)
    tokenized_card_data_create.expiry_date = MOCK_CARD_DATA.card_data.expiry_date

    tcr = PostfinancecheckoutRubySdk::TokenizedCardRequest.new(card_data: tokenized_card_data_create)
    ...
    assert_not_nil(url, "Url should not be null")
    assert_not_equal(url, "", "Url should not be empty")
    assert_not_equal(processed.state, PostfinancecheckoutRubySdk::TransactionState::FAILED, "Transaction state should not be FAILED")
    assert_not_equal(processed.state, PostfinancecheckoutRubySdk::TransactionState::PENDING, "Transaction state should not be PENDING")
  end
  ...
  def test_fetch_terminal_receipts_should_return_valid_terminal_receipts_list
    ...
    receipt_response = @transactions_service.get_payment_transactions_id_terminal_receipts(
      transaction.id, PostfinancecheckoutRubySdk::TerminalReceiptFormat::TXT, 200, SPACE_ID
    )
