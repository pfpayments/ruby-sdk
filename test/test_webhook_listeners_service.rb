...
# postfinancecheckout webhook listeners service tests
class WebhookListenersServiceTest < Test::Unit::TestCase
  ...
  def setup
    @webhook_listeners_service =   PostfinancecheckoutRubySdk::WebhookListenersService.new
    @webhook_urls_service =   PostfinancecheckoutRubySdk::WebhookURLsService.new
  end
  ...
  def test_create_and_delete_webhook_listener
    ...
    assert_equal(  PostfinancecheckoutRubySdk::CreationEntityState::DELETED, deleted_response.state, "State should be DELETED")
  end
  ...
  def test_get_webhooks_listeners_search_without_custom_timeout
    ...
    search_response.data.each do |webhook_listener|
      ...
      assert_equal   PostfinancecheckoutRubySdk::CreationEntityState::INACTIVE, webhook_listener.state,
                   "Expected Webhook Listener State #{webhook_listener.state} to equal #{   PostfinancecheckoutRubySdk::CreationEntityState::INACTIVE}"
    end
  end
  ...
  def test_get_webhooks_listeners_search_with_custom_timeout
    ...
    config_with_timeout =   PostfinancecheckoutRubySdk::Configuration.new do |c|
      c.user_id = TEST_USER_ID
      c.authentication_key = TEST_AUTHENTICATION_KEY
      c.timeout = 36
    end
    api_client_with_timeout =   PostfinancecheckoutRubySdk::ApiClient::new(config_with_timeout)
    webhook_listener_service_with_timeout =   PostfinancecheckoutRubySdk::WebhookListenersService.new(api_client_with_timeout)
  ...
  def private_webhook_listener_create(webhook_url_id)
      PostfinancecheckoutRubySdk::WebhookListenerCreate.new(
      {
        name: WEBHOOK_LISTENER_NAME,
        state:   PostfinancecheckoutRubySdk::CreationEntityState::ACTIVE,
